...
- [x] 保留旧“双一致性案例工作台”，并通过 Web 的 `--openjml` 参数或 `OPENJML` 环境变量配置 OpenJML；它仍不属于 JML–JML 正式评分。
- [x] 一致性 CLI 默认将提交 JML 临时绑定到目标 Java 方法，记录输入哈希、绑定摘要和 OpenJML 定位；旧演示可用 `--embedded-java-contract` 显式运行内嵌合同模式。
